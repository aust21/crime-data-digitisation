...
from data.fetch import fetch_aggrigated_by_cat, fetch_highest_count, fetch_min_count
...
option = st.selectbox(
    "Select Aggregation",
    ["Original Stats", "Aggregated by category", "Highest Crime Count", "Lowest Crime Count"]
)
...
if option == "Original Stats":
    ...
    df_result = fetch_highest_count(original_file)
elif option == "Lowest Crime Count":
    df_result = fetch_min_count(original_file)
